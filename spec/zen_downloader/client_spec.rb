...
RSpec.describe ZenDownloader::Client do
  ...
  describe "#login" do
    ...
    context "when login succeeds" do
      before do
        ...
        title_element = instance_double(Ferrum::Node, text: "ZEN University")
        ...
        allow(mock_browser).to receive(:current_url).and_return("https://www.nnn.ed.nico/")
      end

      it "completes the login flow" do
        page = client.login

        expect(page.title).to eq("ZEN University")
      ...
      it "accesses the ZEN ID auth page directly" do
        expect(mock_browser).to receive(:go_to).with("https://www.nnn.ed.nico/auth/zen_id")
  ...
  describe "#fetch_page" do
    ...
    context "when not logged in and session invalid" do
      let(:email_field) { instance_double(Ferrum::Node) }
      let(:password_field) { instance_double(Ferrum::Node) }
      let(:submit_button) { instance_double(Ferrum::Node) }
      let(:mock_cookies) { instance_double(Ferrum::Cookies) }

      before do
        allow(mock_browser).to receive(:go_to)
        allow(mock_browser).to receive(:cookies).and_return(mock_cookies)
        allow(mock_cookies).to receive(:all).and_return({})
        allow(mock_cookies).to receive(:set)

        # session_valid? returns false (user API returns error)
        allow(mock_browser).to receive(:evaluate_async).and_return({ "error" => "Unauthorized" })
